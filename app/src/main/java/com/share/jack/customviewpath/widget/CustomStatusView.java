...
public class CustomStatusView extends View {
  ...
  public CustomStatusView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray array = context.getTheme()
        .obtainStyledAttributes(attrs, R.styleable.CustomStatusView, defStyleAttr, 0);
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    canvas.translate(getPaddingLeft(),
        getPaddingTop());   //将当前画布的点移到getPaddingLeft,getPaddingTop,后面的操作都以该点作为参照点
    if (mStatus == StatusEnum.Loading) {    //正在加载
      ...
      canvas.drawArc(new RectF(0, 0, progressRadius * 2, progressRadius * 2), startAngle,
      ...
      mPathMeasure.getSegment(0, circleValue * mPathMeasure.getLength(), mPathCircleDst,
          true); //截取path并保存到mPathCircleDst中
      canvas.drawPath(mPathCircleDst, mPaint);

      if (circleValue == 1) {//表示圆画完了,可以钩了
        successPath.moveTo(getWidth() / 64 * 16, getWidth() / 64 * 34);
        successPath.lineTo(getWidth() / 64 * 28, getWidth() / 64 * 47);
        successPath.lineTo(getWidth() / 64 * 52, getWidth() / 64 * 26);
